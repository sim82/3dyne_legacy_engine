...
//	};
    ...
    node match_gs_expression( token_stream *ts ) {
        rule_t term_primary = [&](token_stream *ts) { return match_term_primary(ts);};
        rule_t assignment = match_list({ match_name(), match_other( "="), term_primary },"assignment");


        rule_t command = match_list({match_name(), match_other("("), match_other(")")},"command");
        rule_t sel = match_any({assignment,command});

        return sel(ts);
    }

    node match_gs_expression_list( token_stream *ts ) {
        rule_t exp = [&]( token_stream *ts ) {return match_gs_expression(ts);};
        node n = exp(ts);
        ...
        return match_all_from_left( ts, n, match_list( {match_other(";"), exp }, "expr_list", 1));
    }
...
{
    const size_t size = strlen(buf_start);
    const char *buf_end = buf_start + size;


    typedef lexer<const char *> my_lexer;


    my_lexer l( buf_start, buf_end );
...
void interpreter::dispatch(const std::string &name )
{
    ...
    if( it != sender_map_.end() ) {
        it->second(target_queue_);

    }
}

gs::variant::operator int() const {
    return stoi( *this );
}

gs::variant::operator double() const {
    return stof( *this );
}

gs::variant::operator const std::string &() const {
    return *this;
}
